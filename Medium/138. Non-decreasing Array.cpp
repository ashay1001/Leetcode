    ...
    bool checkPossibility(vector<int>& nums) {
        ...
        for(int i = 1; i < nums.size(); i++){
            ...
            if(nums[i] < nums[i-1]){
                ...
                //if prev of prev element of curr is less than or equal to it then only the we can make it non decreasing by 
                //making the greater equal to lesser (prev = curr).
